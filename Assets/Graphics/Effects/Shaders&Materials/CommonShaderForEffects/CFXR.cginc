...
//--------------------------------------------------------------------------------------------------------------------------------
	...
	float SoftParticles(float near, float far, float4 projection)
	{
		...
		void vert(MeshData v, v2f_shadowCaster o, out float4 opos)
	#else
		Interpolators vert(MeshData v, Interpolators o)
	#endif
		{
	...
	#else
		half4 frag(Interpolators i, half3 particleColor, half particleAlpha, half dissolve, half dissolveTime) : SV_Target
	#endif
		{
			//Blending
